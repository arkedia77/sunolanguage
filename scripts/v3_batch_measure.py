...
def main(tags):
    ...
    if need <= 0:
        ...
        for name, share in (("등록 가정 25%[판정]", 0.25),
            ...
            REG_N = 309
            # ★★2026-09-14 봉인 — 사전등록 검정은 **1회만** 집행한다.
            #   ⛔실물: n=320(N074)에서 집행 ⇒ 결론 **「가르지 못함」**(예상 9.7%·귀무 5.7% 둘 다 CI 안).
            #     그런데 다음 배치 n=340(N075)에서 귀무가 CI 밖으로 나가자 이 도구가
            #     **「귀무 기각」**을 찍었다. ⇒ ★**매 배치 다시 검정하면 「넘을 때까지 두드리는 것」**이다.
            #   ★경계에서 실제로 오갔다: n=280 밖 · n=300 안 · n=320 안 · n=340 밖.
            #   ⇒ 집행 시점의 결론을 **봉인**하고, 이후 배치는 **수만** 적는다.
            SEAL = {"n": 320, "batch": "N074", "실측": "26/320 = 8.12%", "CI": "5.6~11.6",
                    "검정력": "81%", "결론": "⛔가르지 못함(예상 9.7%·귀무 5.7% 둘 다 CI 안)"}
            if "참고" in name:
                verdict = ""
            elif len(rows) > SEAL["n"]:
                verdict = (f" → ⛔**판정 종료** · 사전등록 검정은 n={SEAL['n']}({SEAL['batch']})에서"
                           f" «1회» 집행됨 ⇒ {SEAL['결론']}"
                           f"\n       ⛔이후 배치는 «수만» 적는다 — 매 배치 재검정은 «넘을 때까지 두드리기»다."
                           f"\n       ★경계에서 실제로 오갔다: n=280 밖 · n=300 안 · n=320 안 · n=340 밖.")
            elif len(rows) < REG_N:
                verdict = (f" → ⛔미판정(등록선 클립 {REG_N} 미도달: 현재 {len(rows)}"
                           f" · 검정력 {100*pw:.0f}%)")
            else:
                # ★2026-09-14 재개정 — 등록선에 «도달했는데» 이 도구가 「유지」를 찍었다.
                #   실물: 실측 8.12%(CI 5.6~11.6)에 **예상 9.7%와 귀무 5.7%가 «둘 다» 들어왔다.**
                #   ⇒ 그건 「내 예측이 유지됐다」가 아니라 ★**「어느 쪽도 기각 못 했다」**다.
                #   ⛔검정력 81%는 «9.7%↔5.7%를 가르는» 힘이지, 그 «사이» 값이 나왔을 때
                #     가려 주는 장치가 아니다 — 실측이 두 값 사이에 떨어지면 힘이 차도 안 갈린다.
                #   ★「도구가 대신 단정한다」의 여섯 번째다. 읽는 사람은(나 포함) 낱말을 집는다.
                null_out = not (lo <= 5.7 <= hi)
                if out and null_out:
                    verdict = " → ⛔둘 다 CI 밖 — 「예측이 틀렸다」와 「기저가 다르다」가 같이 참"
                elif out:
                    verdict = " → 반증(예상만 CI 밖 · 귀무는 기각 못 함)"
                elif null_out:
                    verdict = " → 귀무 기각(예상은 CI 안 — ★「예측이 맞다」가 아니다)"
                else:
                    verdict = (" → ★⛔**가르지 못함** — 예상·귀무가 «둘 다» CI 안."
                               " 「유지」라 쓰지 않는다(기각 실패이지 지지가 아니다)")

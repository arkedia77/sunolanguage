...
| 사전 최신 | **v3.4 (2026-09-02, 619트랙 기준)** — 코퍼스 증분 반영(589→619트랙·단어 5520→5649·장르 283→293). 큐레이션 보존 **78건**·축소 0·pytest 88 passed (백업 `rag/suno_dictionary_v3.json.bak_v3.3`). ⚠**층 경고(선행 결함·v3.3에도 있었음)**: `mood_emotion`·`timbre_texture`·`tempo_rhythm` 3섹션은 `words.freq_sp`를 읽는데 `lexical_search_cli.py:42`가 `leomusic_sp_full`(**우리 입력층**)을 `'sp'`로 접어 넣는다 ⇒ 그 3섹션 68단어의 phrase 출현 **9,228 중 3,052(33.1%)가 입력층**이고 `nostalgic` 49/49·`dreamy` 33/33·`emotional` 360/361·`ethereal` 34/35는 **사실상 전량 우리 문자열**이다. ★slot 기반 나머지 섹션(instrument/drum/technique/production/harmony/dynamics/vocal/genre)은 입력층 slot이 비어 있어 **기여 0**이라 무관. ⛔**커넥터 OUT 재발행은 이 건 판정 전까지 보류**(5팀 전파 대상). |
| 사전 재빌드 카운터 | **0곡** (측정 2026-09-02: `corpus_ingest_runner.py record-rebuild --apply` = merged 560곡 − lexical 619트랙 차집합 → 미포함 **0곡**. 상태DB `dict_version` v3.3→v3.4 · `last_rebuild_at` 08-15→09-02 · `rebuild_counter` 30→0 · `lexical_entries` 19,084→20,456 기록 완료. `corpus_health_check --no-net` = **PASS 6/6·WARN 0**) — 다음 트리거: 누적 ≥30곡 / thin 장르 ≥5 진입 / 마지막 재빌드 후 90일+누적 ≥10곡 |

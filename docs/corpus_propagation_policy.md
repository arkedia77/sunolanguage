...
| 사전 재빌드 카운터 | **0곡** (측정: `corpus_ingest_runner.py record-rebuild` = merged 530곡 − lexical 589트랙 차집합 → 미포함 **0곡**. ★08-16 기계 기록 완료 — 그 전까지 상태DB는 `33`이었고 이 칸의 「리셋」은 **문서만의 선언**이었다) — 다음 트리거: 누적 ≥30곡 / thin 장르 ≥5 진입 / 마지막 재빌드 후 90일+누적 ≥10곡 |

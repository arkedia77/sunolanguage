...
**최종 점검 2026-09-13 (코퍼스 5줄 점검 3회차)** — ⛔**이 날짜는 「전 칸 재측정일」이 아니다**: 값의 측정일은 **각 칸 괄호 안**에 따로 적혀 있고 서로 다르다. ★값에 단위·층을 붙여 쓴다(곡/트랙/entries는 서로 다른 층). 괄호 안은 **측정법**(재현 가능한 것만 적는다).
...
| 표현 레이어 | **453개념 / 2,718표현(6레지스터×453) / 인바운드 별칭 72** (측정 2026-09-14 집행 후: `sunolang.db` `expr_concepts`·`expr_expressions`·`expr_inbound_aliases` 각 count(*)) · ✅**저작 커버리지 = 원자 453 / 저작 453 / 미저작 0**(`build_expression_db.py --coverage`) · `select distinct dict_version from expr_concepts` = **`3.5` 단일**(사전과 일치 — 09-14 이전엔 `3.3`에 멈춰 있었다). ★**2026-09-14 오너 결재로 미저작 7원자 × 6레지스터 = 42표현 증분 저작 집행**(446→453개념 · 2,676→2,718표현 · 별칭 72 불변 · 예측값과 정확히 일치). 저작분 = `ring out`·`return`·`builds from`·`half-time feel, filtered synth`·`full band, energetic delivery`·`full band, brass section enters`·`a prominent brass section with trumpets and saxophones, a slap bass guitar`. ⚠**`ring out`/`return`은 기존 개념 `rings out`/`returns`의 «어형 변종»**이다 — 정본 키(slugify)는 하이픈·공백·대소문자만 접고 **어미 변화는 안 접으므로** 별개 원자로 잡힌다. ⛔**중복으로 보고 한쪽을 지우지 말 것**(지우면 사전 재빌드가 다시 만든다) — 각 entry `notes`에 박아 두었다. `builds from`은 전치사로 끝나는 조각이라 **confidence=low**로 정직 표기. 검증: `--coverage` 미저작 0 · `corpus_health_check` **H6·H6-db PASS(종합 8/8·WARN 0)** · `pytest` **88 passed** · 검색기 스모크 통과 · 빌드 전 DB 백업 `sunolang.db.bak_pre_expr_author_20260914`. — ★**정본은 DB가 아니라 `data/expressions/authored/*.json` + `inbound_aliases_seed.json`**(이번 저작도 **파일에 먼저 쓰고** `--build`로 파생시켰다). DB는 파생물이라 **DB에만 쓴 값은 재빌드로 조용히 소실된다**(08-15 별칭 6건 실물) |
...
| 커넥터 OUT | **interface v0.2 / snapshot `cs-3.3-589-20260815` / 구독 5팀** (측정: `python3 scripts/corpus_connector.py status`. 산출 `data/connector/out/crosswalk_v0.2.json` sha256 `d1c4e9ad48b4fcfb…` · `breaking_content=false`=additive) — 소비자=leomusic·leomusic2·leomusic3·leomusic-trot·**encore**(08-15 편입) ⚠**2026-09-13 실측: 스냅샷이 현행 사전과 불일치** — `cs-3.3-589-20260815`(dict v3.3·589트랙·446개념) ↔ 현행 **v3.5·619트랙·453개념**(09-14 저작으로 개념 격차가 7 더 벌어졌다). 09-02 층 분리분 **미발행 11일째**(09-02 기록의 「OUT 재발행 미집행·전파 전 고지 필요」가 그대로 열려 있음). ⛔**지금 그대로 발행하면 08-15과 같은 「라벨만 새 버전」 사고**가 되고, 이번엔 내용까지 바뀐다(프로듀서가 쓰던 어휘 12개가 「Suno 관측 0」으로 표시) ⇒ **재발행 = 고지 동반·오너 결재 대기**. ★**2026-09-19부터 이 불일치는 손이 아니라 기계가 본다** — `corpus_health_check` **H7**(발행 스냅샷 ↔ 현행 코어 스냅샷 · 마지막 OUT 발행일·경과일 동반). ⛔**경과일을 이 표에 숫자로 안 적는다** — 적는 순간 늙고, 자가 두 벌이 된다. |

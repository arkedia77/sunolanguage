...
| 표현 레이어 | **446개념 / 2,676표현(6레지스터×446) / 인바운드 별칭 72** (측정 2026-09-13 재확인: `sunolang.db` `expr_concepts`·`expr_expressions`·`expr_inbound_aliases` 각 count(*)) / ⚠**저작 커버리지 = 사전 v3.5 원자 453 − 저작 446 = 미저작 7** (측정 2026-09-13: `build_expression_db.py --coverage`) ⇒ ★**표현 레이어는 v3.3 원자 기준에 멈춰 있다** — 근거는 라벨이 아니라 DB 자체 필드: `select distinct dict_version from expr_concepts` = **`3.3` 단일**(사전은 v3.5). 미저작 7건 = `ring out`·`return`·`builds from`·`a prominent brass section with trumpets and saxophones, a slap bass guitar`·`half-time feel, filtered synth`·`full band, energetic delivery`·`full band, brass section enters` — **v3.3→v3.4 재빌드(09-02)가 들여온 신규 원자 7건과 정확히 일치·소실 0** (측정: 백업 사전 `.bak_v3.3`/`.bak_v3.4`로 `extract_atoms()` 재실행 후 차집합. v3.4→v3.5는 원자 불변 = 층 분리가 키를 안 지웠다는 기록과 합치). ⛔**컴파운드 4건을 「원자화 결함」으로 보고 빼지 않는다** — 기존 저작 446 중 **113건이 이미 같은 다낱말 모양**이고 `full band, high energy`·`full band enters`·`half-time feel`이 실제로 저작돼 있다(관행상 저작 대상이 맞다). ★**미저작 상태는 2026-09-13 기계 점검에 편입**(`corpus_health_check` **H6/H6-db** 신설 — 그전엔 손 점검 ⓒ뿐이라 **11일간 H1~H5가 PASS 6/6**이었다). 저작 잔여 = **7원자 × 6레지스터 = 42표현**(집행 시 2,676→2,718·개념 453) — **오너 결재 대기**(08-15 전례: 9원자×54표현이 결재 아래 집행). — ★**정본은 DB가 아니라 `data/expressions/authored/*.json` + `inbound_aliases_seed.json`**. DB는 파생물이라 **DB에만 쓴 값은 재빌드로 조용히 소실된다**(08-15 별칭 6건이 실제로 그렇게 사라졌다가 정본 복원됨) |

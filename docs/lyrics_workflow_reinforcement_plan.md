...
**현재**: 섹션마다 `genre + mood + role_hint` 평면 쿼리. verse2만 직전 텍스트 주입
**보강**: 곡 단위 아크 플랜 도출(예 발라드: setup→longing→climax→resolution) →
각 섹션 쿼리에 아크-스테이지 키워드를 블렌딩. `prev_verse_text` 주입을 일반화하여
**모든 섹션**이 "story-so-far"(선택된 섹션 키워드 누적) 앵커를 받음.
- `lyrics_themes.py`의 sub_theme 메커니즘과 합류, arc 어휘 테이블 신설
- **효과**: 검색이 독립→누진(progressive)으로 전환. "이야기 전개"의 구조적 토대

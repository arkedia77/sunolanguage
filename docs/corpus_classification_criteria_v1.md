...
→ ★**08-15: 그 대조의 설계·사전등록을 저작함** = `docs/duration_lock_control_preregistration.md`(락 ON/OFF × 분량 2수준, 재는 것은 **상호작용**). ★**생성은 안 열었으므로 등급은 C 그대로다 — 설계가 대조군을 만들지 않는다.** 같은 문서 §3.1에서 **내 σ 추정법을 정정**했다(폭÷4는 n≈10~25를 가정한 나눔수. n=2 자료엔 d2=1.128 ⇒ **σ̂ 22.8초**, 이전 추정의 3.5배).

...
> ★**관측층·스냅샷 한정자**(2026-08-17): 이 장의 수치는 **출력층**(Suno가 완성곡을 듣고 쓴 서술) 관측이고, **입력층은 미인덱스**라 비교 대상이 아니다 → [2장 §2.0](ch2_two_channels.md). 「0건」을 읽는 법 → [5장 §5.0](ch5_absence.md). 본문의 **437곡·385행·사전 v3.1은 구판 스냅샷**이며 현행은 **파일 코퍼스 530곡 / lexical 589트랙 / 19,084 entries / 283장르 / 사전 v3.3**이다 — ★곡·행·트랙·entries는 **서로 다른 층**이라 혼용 금지(589를 「590곡」으로 인용하면 틀린다).
...
`key of X` 패턴은 652회 출현하고, 구체적 코드명(Am7, Cmaj7)은 출력층 **1곡**, 로마 숫자 진행 표기(I-IV-V)는 **0곡**이다 — **이름을 붙인 화성**은 거의 안 돌아온다. ⚠단 「Suno의 화성 인식은 조성 수준에서 멈춘다」는 **2026-08-22 반증됐다**: `chords` 출력층 **269곡**(코퍼스의 51%)·`chord progression`(단수) **28곡**. 멈추는 것은 인식이 아니라 **명명**이다(§5.1 정정표).

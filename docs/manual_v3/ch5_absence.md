...
## ★5.0 이 장의 「0건」을 읽는 법 — 채널·층 한정자 (2026-08-17 신설)

이 장은 **「없다」를 다루는 장**이라 다른 어느 장보다 한정자가 중요하다. 실제로 이 장 계열의 표 하나가 **남의 커미션 설계를 닫을 뻔했다.**

### 규칙 1 — ★채널 안 적은 dead는 dead가 아니라, 어느 채널인지 안 적은 것

**실사고(2026-08-11)**: 내가 만든 표에 `spoken = dead`가 채널 한정자 없이 실렸다. 그 dead는 **브래킷 `[]` 채널 한정**이었는데, 표만 인용되면서 「나레이션 생성 불가」로 읽혔다. 실제 판정은 채널마다 다르다:

| 채널 | `spoken` 판정 | 근거 |
|---|---|---|
| `[spoken]` 브래킷 단독 | **dead** | 고립 브래킷은 안 먹음 |
| SP 산문 | **live** | 관측 9곡 / 16문장형 |
| `(spoken)` 괄호 | **live** | Leo 실청취 4/4 |

문법 핵심: spoken-word는 **독립 블록이 아니라 `transitioning into` 전이로 표상**된다 — 고립 `[spoken]`이 죽는 이유가 이것이다.

⇒ **dead/live/0건을 적을 때는 채널을 값 옆에 병기한다.** 본문에만 caveat을 달고 표에는 안 다는 것도 실패다 — ★**표만 떼어 인용되면 caveat은 따라가지 않는다.**

### 규칙 2 — ★이 장의 「0건」은 **출력층** 0건이다

이 장의 집계는 전부 `suno_reanalysis`(Suno가 완성곡을 듣고 쓴 서술) 기준이다. 입력층(우리가 써넣은 `leomusic_original`)은 **어떤 인덱스에도 안 들어가 있다** — 브래킷 3,725개·고유 923·393곡 미인덱스(2026-08-17 실측, 자세히는 [2장 §2.0](ch2_two_channels.md)).

⇒ 여기 적힌 「0건」은 정확히는 **「출력층에서 0건」**이다. 입력층에서 0건이라는 뜻이 **아니다** — 입력층은 애초에 세어 본 적이 없다. ★**「없음」과 「안 봄」은 다르고, 그 둘을 가르는 것이 이 장의 본업이다.**

### 규칙 3 — 0건은 「불가」가 아니라 「이 자로는 못 잼」일 수 있다

표본이 없어 0인 것과, 시도했는데 0인 것은 다르다. 검정력이 없으면 **「판정 불가」로 적고 0으로 접지 않는다**(요청 표본 0~14곡에서 기대 동시 0.0~0.9곡이면 애초에 못 재는 구간이다).

### 수치 스냅샷 주의

본문의 **「437곡」·「385행 DB」·「사전 v3.1」은 구판**이다. 현행(2026-08-17) = **파일 코퍼스 530곡 / lexical 589트랙 / 19,084 entries / 283장르 / 사전 v3.3 / 표현 446개념·2,676표현·별칭 72**. ★표현 레이어의 **정본은 파일**(`data/expressions/authored/*.json`)이고 **DB는 파생**이다 — DB에만 쓴 값은 재빌드가 조용히 지운다.

---

## 5.1 완전 데드존 — 0건 카테고리

> ★이 절의 「0건」은 전부 **출력층 기준**이며, 채널 한정자는 각 항목에서 확인할 것(§5.0 규칙 1·2).

### 코드명과 코드 진행

> ⚠**2026-08-22 전면 정정.** 이 절의 옛 문면은 「코드 진행 0건 / Suno의 화성 인식은 조성 수준에서 멈춘다」였다.
> **반증됐다.** 아래는 `lexical_index.sqlite` v3.3(589곡) 재실측이며, 재현은 `scripts/w030_harmony_probe.py`.
> 이 오기재는 자매 프로젝트로 전파돼 실피해를 냈다 — 아래 「전파 경위」 참조.

**★가르는 선은 「코드를 말하는가」가 아니라 「코드에 이름을 붙이는가」다.**

| 대상 | 출력층(Suno가 말함) | 입력층(우리가 씀) |
|---|---|---|
| `chords` (복수) | **269곡** ← 코퍼스 530곡의 **51%** | 118곡 |
| `chord` (단수) | **51곡** | 57곡 |
| `chord progression` (단수) | **28곡** (sp_entity 26 · 브라켓 5 · 스템 7) | 7곡 |
| `chord progressions` (복수) | **0곡** | 12곡 |
| 구체적 코드명 (Cmaj7·Gmaj7·Am7…) | **1곡** (브라켓 `plays a final Gmaj7 chord and rings out`) | 6곡 |
| 로마 숫자 진행 (I-IV-V, ii-V-I) | **0곡** | 4곡 |
| `key of X` 패턴 | 652회 | — |

- **여전히 참인 것**: 구체적 **코드명**(1곡)과 **로마 숫자 진행 표기**(0곡)는 출력층에서 사실상 안 나온다.
  둘 다 입력층엔 우리가 써왔으므로(6곡·4곡) **「처방했으나 안 돌아옴」 = L3 dead-zone 정의에 맞는다.**
- **틀렸던 것**: 「코드 진행 0건」·「화성 인식이 조성에서 멈춘다」. Suno는 코드를 **아주 많이** 말한다
  — `a jazz-influenced chord progression with subtle chorus and reverb` 같은 **질감 서술**로. 이름만 안 붙일 뿐이다.
- ⇒ SP에 쓸 수 있는 것: `chord progression`(단수)·`chords`·`seventh chords` 같은 **무명(無名) 화성 서술**.
  쓸 수 없는 것: `Am7 → Dm7 → G7`·`ii-V-I` 같은 **기호 표기**.

**★전파 경위 (같은 실수를 반복하지 않기 위해 남긴다)**

1. 2026-06-15, 우리가 leomusic-trot에 SP 검수를 보내며 `"progressions": "25곡, 0건 — 'chord progressions' → 삭제"`라고 적었다.
   **표제어는 복수형이었고 그 수치는 옳았다**(복수형 출력층 0곡 — 오늘 재확인).
2. 그런데 그 옆에 **`★Suno 코드진행 미렌더(우리 5장 데드존 재확인)`**라는 **일반화 해설**을 붙였다. 근거는 복수형 25곡인데
   해설은 코드 진행 전반을 부정했다. 그 해설의 출처가 바로 이 절의 옛 문면이다.
3. leomusic-trot 구현자는 그 해설을 읽고 `chord progressions?`(정규식 `s?`)로 **단수까지** strip 했다.
   ⇒ attested 28곡짜리 표현이 2026-06-15부터 2026-08-22까지 그쪽 SP에서 조용히 지워졌다.
4. 2026-08-22 그쪽 W030 선조회에서 역질의가 들어와 적발됐고, 그쪽은 복수형만 strip 하도록 개정했다.

★**교훈: 수치 옆에 붙인 한 줄 해설이 수치보다 멀리 간다.** 「0건」을 적을 때는 **무엇의 0건인지**(단수/복수·층·채널)를
해설문에도 똑같이 박아야 한다. 표제어만 정확하고 해설이 넓으면, 읽는 쪽은 해설을 구현한다.
...
- **구체적 코드명·기호 진행**: `Am7 → Dm7 → G7 → Cmaj7`·`ii-V-I` 같은 **기호 표기**를 SP에 입력해도 Suno는 이를 무시하고 자체 화성을 생성(출력층 코드명 1곡·로마숫자 0곡). ★단 **무명 화성 서술은 다르다** — `chord progression`(단수)은 출력층 28곡, `chords`는 269곡이다(§5.1 정정표)
...
1. **코드「명」과 기호 진행을 넣지 마라** — 출력층 코드명 1곡·로마숫자 진행 0곡. ★단 **「화성 인식이 조성에서 멈춘다」는 2026-08-22 반증됐다** — `chords` 269곡·`chord progression`(단수) 28곡. 이름 없는 화성 서술은 살아 있다(§5.1)

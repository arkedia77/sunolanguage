...
| `()` 괄호 | `(spoken)` | ✅ **실청취 4/4**(Dead Budget 04-25·별개 재료) + ASR 재현 — ★**입력층 실측이라 이 표에서 등급이 가장 높다** ⚠**단 기계 판별은 2클립 중 1클립만 견고**(RM2 대조군 15조합 중 14가 「말하기」 / RM1은 대조군 종속이라 판정 불가). ★**N=2 — 곡 수준 일반화 금지.** 근거=`scripts/spoken_probe_sensitivity.py` | 해당 행을 말로 |
